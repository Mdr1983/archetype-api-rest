...
import ${package}.application.domain.model.domain.mongo.purchase_order.PurchaseOrderGenericDocument;
...
import ${package}.application.domain.ports.secondary.repository.mongo.GenericDocumentRepository;
import ${package}.application.util.SpringBeanUtil;
...
public class PurchaseOrderGenericDocumentRepository implements GenericDocumentRepository<PurchaseOrderGenericDocument> {
  ...
  @Caching(put = {
      @CachePut(cacheNames = "purchaseOrderGenericDocument",
          key = "{ 'findByCode', ${symbol_pound}result.getData().get('code') }",
          unless = "${symbol_pound}result == null"),
      @CachePut(cacheNames = "purchaseOrderGenericDocument",
  ...
  @Caching(evict = {
      @CacheEvict(cacheNames = "purchaseOrderGenericDocument",
          key = "{ 'findByCode', ${symbol_pound}entity.getData().get('code') }"),
      @CacheEvict(cacheNames = "purchaseOrderGenericDocument",
